    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'flex-end', background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)' }}>
      <div style={{ background: 'white', width: '100%', maxWidth: 430, margin: '0 auto', borderRadius: '24px 24px 0 0', padding: '24px 24px 40px' }}>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          {!isLast && (
            <button onClick={onClose} style={{ flex: 1, padding: 12, borderRadius: 16, border: '1px solid #F0E8E0', background: 'white', color: '#9CA3AF', fontSize: 14, cursor: 'pointer' }}>
              Saltar
            </button>
          )}
          <button
            onClick={() => isLast ? onClose() : setIdx(idx + 1)}
            style={{ flex: isLast ? 1 : 2, padding: 12, borderRadius: 16, border: 'none', background: 'linear-gradient(135deg,#F4B880,#E89860)', color: 'white', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}
          >
