...
// Opacidad progresiva para dots bloqueados
    ...
    <div style={{ position:'fixed', inset:0, zIndex:50, display:'flex', alignItems:'flex-end', background:'rgba(0,0,0,0.6)', backdropFilter:'blur(4px)' }}>
      <div style={{ background:'white', width:'100%', maxWidth:430, margin:'0 auto', borderRadius:'24px 24px 0 0', padding:'24px 24px 40px' }}>
        ...
        <div style={{ display:'flex', gap:12 }}>
          {!isLast && <button onClick={onClose} style={{ flex:1, padding:12, borderRadius:16, border:'1px solid #E5E7EB', background:'white', color:'#6B7280', fontSize:14, cursor:'pointer' }}>Saltar</button>}
          <button onClick={() => isLast ? onClose() : setIdx(idx+1)} style={{ flex:isLast?1:2, padding:12, borderRadius:16, border:'none', background:'#2D6A4F', color:'white', fontSize:14, fontWeight:500, cursor:'pointer' }}>
      ...
      <div style={{ padding:'14px 18px 0' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:6, marginBottom:14 }}>
          {[
            ...
            <div key={label} style={{ background:'white', borderRadius:14, padding:'10px 8px', textAlign:'center', border:'0.5px solid #E5E7EB' }}>
              <p style={{ fontSize:16, fontWeight:500, color:'#111', margin:0 }}>{value}</p>
              <p style={{ fontSize:11, color:'#9CA3AF', margin:0 }}>{label}</p>
            </div>
          ))}
        </div>

        {/* ── TARJETA UNIFICADA: MAPA + SEMANA ACTUAL + CTA ── */}
        <div style={{
          borderRadius:20, overflow:'hidden', marginBottom:12,
          background:'linear-gradient(160deg,#1B4332 0%,#2D6A4F 60%,#1A237E 100%)'
        }}>

          {/* ZONA A — Mapa de los 5 mundos */}
                  ...
                  <div key={week.id} style={{ display:'flex', alignItems:'center', flex:1 }}>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        border: isCurrent ? '2.5px solid white' : '1.5px solid rgba(255,255,255,0.45)',
                        background:'rgba(255,255,255,0.93)',
                        cursor:isLocked?'default':'pointer',
                        display:'flex', alignItems:'center', justifyContent:'center',
                        flexShrink:0, padding:0, overflow:'hidden',
                        opacity,
                        boxShadow: isCurrent ? '0 0 0 5px rgba(255,255,255,0.18)' : 'none',
                        transition:'all 0.2s'
                      }}>
                      {isLocked ? (
              ...
              {COURSE_WEEKS.map(week => {
                ...
                return (
                  <div key={week.id} style={{ flex:1, textAlign:'center' }}>
                    <p style={{
                      fontSize:9, margin:0,
                      color: isCurrent ? 'white' : isCompleted ? 'rgba(255,255,255,0.65)' : `rgba(255,255,255,${LOCKED_OPACITY[i]??0.2})`,
          ...
          <div style={{ padding:'10px 16px 14px' }}>
            <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:10 }}>
              ...
              <div style={{ display:'flex', gap:4 }}>
                ...
                <span style={{ background:'rgba(255,255,255,0.18)', color:'white', fontSize:9, padding:'3px 8px', borderRadius:20 }}>{currentWeekData?.posturas.length} pos</span>
              </div>
            </div>
            ...
            {practicedToday ? (
              <button
                ...
                style={{ width:'100%', background:'rgba(255,255,255,0.14)', border:'1px solid rgba(255,255,255,0.28)', borderRadius:12, padding:'9px 14px', display:'flex', alignItems:'center', justifyContent:'space-between', cursor:'pointer', boxSizing:'border-box' }}>
                <div style={{ display:'flex', alignItems:'center', gap:8 }}>
              ...
              <button
                onClick={() => router.push(`/semana/${currentWeek}`)}
                style={{ width:'100%', background:'rgba(255,255,255,0.18)', border:'1px solid rgba(255,255,255,0.32)', borderRadius:12, padding:'10px 14px', display:'flex', alignItems:'center', justifyContent:'space-between', cursor:'pointer', boxSizing:'border-box' }}>
        ...
        {/* ── COACH DE KAWA — sin label, solo emoji + texto ── */}
